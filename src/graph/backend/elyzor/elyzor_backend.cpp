...
size_t elyzor_backend_t::get_mem_size(const logical_tensor_t &lt) const {
...
graph::pass::pass_registry_t elyzor_backend_t::register_passes() {
...
graph::pass::pass_registry_t elyzor_backend_t::pass_registry_
        = elyzor_backend_t::register_passes();

status_t elyzor_backend_t::get_partitions(
    ...
    const bool disable_elyzor_bkd
            = graph::utils::getenv_int_internal("DISABLE_ELYZOR_BACKEND", 0)
            > 0;
    if (disable_elyzor_bkd) return status::success;
...
void register_elyzor_backend() {
    backend_registry_t::get_singleton().register_backend(
            &elyzor::elyzor_backend_t::get_singleton());
}
